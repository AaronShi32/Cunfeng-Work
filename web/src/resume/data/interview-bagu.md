...
| **锁粒度** | 行锁（+表锁） | 表锁 |
| **外键** | ✅ 支持 | ❌ 不支持 |
| **崩溃恢复** | ✅ redo log 保证（crash-safe） | ❌ 容易损坏 |
| **索引结构** | 聚簇索引（数据和主键索引在一起） | 非聚簇索引（索引和数据分离） |
| **COUNT(\*)** | 需要遍历（MVCC 下无法缓存精确值） | 存储了行数，直接返回，极快 |
| **MVCC** | ✅ 支持（多版本并发控制） | ❌ 不支持 |
| **全文索引** | ✅（5.6+） | ✅ |
| **适用场景** | 读写混合、高并发、数据一致性要求高 | 读多写少、不需要事务（日志表、统计表） |

**MySQL 5.5+ 默认引擎是 InnoDB**，生产环境几乎全用 InnoDB。
...
| 问题 | 现象 | 原因 | 解决方案 |
|---|---|---|---|
| **缓存穿透** | 大量请求查询 **不存在的数据**，缓存和 DB 都没有，每次都打到 DB | 恶意攻击或业务异常，查询 id=-1 等非法数据 | ① 缓存空值（设短 TTL）② **布隆过滤器**拦截不存在的 key ③ 参数校验前置 |
| **缓存击穿** | **单个热点 key** 过期瞬间，大量并发请求同时打到 DB | 热点数据过期 + 高并发 | ① **互斥锁**（setnx），一个线程回源其余等待 ② 热点 key 永不过期，异步后台刷新 ③ 逻辑过期（value 里带过期时间，业务层判断） |
| **缓存雪崩** | **大批 key 同时过期** 或缓存服务宕机，请求全部打到 DB，DB 被压垮 | 批量 key 设了相同 TTL / Redis 节点挂了 | ① TTL 加随机值打散过期时间 ② Redis 高可用（Sentinel / Cluster）③ 本地缓存兜底（Caffeine）④ 限流降级 |

**一句话区分**：穿透是查 **不存在** 的数据；击穿是 **单个热点** key 过期；雪崩是 **大批** key 同时过期或缓存整体不可用。

**面试建议回答**：穿透是查不存在的数据绕过缓存直打 DB，用布隆过滤器或缓存空值解决；击穿是单个热点 key 过期瞬间并发打 DB，用互斥锁或永不过期解决；雪崩是大批 key 同时过期或 Redis 宕机，用 TTL 加随机值打散 + Redis 高可用 + 本地缓存兜底解决。核心区分：穿透看"数据存不存在"，击穿看"热点 key 过不过期"，雪崩看"是否大面积失效"。
...
| 模型层级 | TCP/IP 层级 | 主要协议 | 作用 |
|---|---|---|---|
| **应用层** | 应用层 | HTTP/HTTPS、DNS、FTP、SMTP/POP3/IMAP、SSH、DHCP | 为应用提供服务 |
| **表示层** | 应用层 | SSL/TLS、JPEG、ASCII、GZip | 数据格式转换、加密压缩 |
| **会话层** | 应用层 | RPC、NetBIOS | 建立/管理/终止会话 |
| **传输层** | 传输层 | **TCP**（可靠、面向连接）、**UDP**（不可靠、无连接） | 端到端通信、流量控制 |
| **网络层** | 网络层 | **IP**、ICMP（ping）、ARP、OSPF、BGP | 路由寻址、分组转发 |
| **数据链路层** | 网络接口层 | Ethernet、PPP、VLAN、STP | 帧传输、MAC 寻址 |
| **物理层** | 网络接口层 | 光纤、双绞线、Wi-Fi（802.11） | 比特流传输 |

面试中常问的是 **TCP/IP 四层模型**，OSI 七层了解即可。重点掌握：HTTP（应用层）、TCP/UDP（传输层）、IP/ICMP/ARP（网络层）。

**面试建议回答**：OSI 七层从上到下是应用、表示、会话、传输、网络、数据链路、物理层，实际用的是 TCP/IP 四层模型。核心协议——应用层 HTTP/DNS/SSH，传输层 TCP（可靠面向连接）和 UDP（不可靠无连接），网络层 IP 负责寻址路由、ICMP 用于 ping 探测、ARP 做 IP 到 MAC 的映射。

</details>

---

### 29. WebSocket、SSE 与短轮询、长轮询的区别？
...
|---|---|---|---|---|
| **连接模型** | 请求完立即关闭 | 请求挂起直到有数据或超时 | 持久 HTTP 连接，服务端推送 | 全双工长连接（Upgrade 握手） |
| **方向** | 单向（客户端拉） | 单向（客户端拉，模拟推） | 单向（服务端 → 客户端） | 双向 |
| **协议** | HTTP | HTTP | HTTP/HTTPS | ws:// / wss:// |
| **延迟** | 高（轮询间隔决定） | 中（有数据立刻返回） | 低 | 极低 |
| **服务端复杂度** | 低 | 中（需挂起请求） | 低（原生支持） | 高（需维护连接状态） |
| **服务端连接数** | 低（请求即断） | 中（并发长连接） | 中（并发长连接） | 高（长连接 × 并发用户） |
| **断线重连** | 自然（下次轮询即重连） | 自然（超时后重连） | 浏览器自动重连 | 需手动实现 |
| **典型场景** | 简单状态查询、低频更新 | 消息通知、低实时性推送 | AI Chat 流式输出、新闻 feed | 聊天室、游戏、协同编辑 |
| **代表案例** | 旧版邮件检查 | 微信网页端早期版本 | ChatGPT 回答流式输出 | Slack、飞书消息 |

**一句话记忆**：
- 短轮询 → 不停敲门问"有没有"
- 长轮询 → 敲门站在那等，有了再告诉你
- SSE → 开一扇单向广播窗口，服务端随时往里推
- WebSocket → 打开一条双向电话线，随时互发
