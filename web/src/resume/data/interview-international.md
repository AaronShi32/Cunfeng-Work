...
3. 竞品全景：Databricks / Snowflake / 云厂商
4. 数据产品出海：技术挑战与架构模式
5. 中国数据产品出海现状
6. 我的经验映射
...
|------|-----|-------|-----|--------|--------|
| **托管 Hadoop/Spark** | EMR | HDInsight 📉 | Dataproc | E-MapReduce | EMR |
| **数仓** | Redshift | Synapse 📉 → Fabric | BigQuery | MaxCompute | CDWPG / TCHouse |
| **流处理** | Kinesis + MSK | Event Hubs + Fabric RTI | Pub/Sub + Dataflow | Flink 实时计算 | Oceanus (Flink) |
| **数据集成/开发** | Glue | ADF → Fabric DF | Dataflow | DataWorks | **WeData** |
| **数据湖** | S3 + Lake Formation | OneLake | Cloud Storage + BigLake | OSS + 数据湖 | COS + DLC |
| **ML** | SageMaker | Fabric DS + Azure AI | Vertex AI | PAI | TI Platform |
| **BI** | QuickSight | Power BI ✨ | Looker | Quick BI | DataInsight |
| **统一平台** | ❌ 各自独立 | ✅ Fabric | ❌ 各自独立 | 部分整合 | **WeData 为核心整合** |
...
| 流处理 | Flink 实时计算 | 托管 Apache Flink |
| 日志分析 | Log Service (SLS) | 日志采集与分析 |
| BI 报表 | Quick BI | 50+ 数据源 |
| ML 平台 | PAI | 训练 + 推理 + AutoML |
| AI 模型 | Qwen Cloud | 2026.05 全球上线 |

> 重点客户：GCash(菲) · Mekari(印尼) · TNG(马来) | 投入：RMB 3800亿 / 3年云+AI基建

### 5.2.1 腾讯云数据平台产品矩阵

| 领域 | 产品 | 说明 |
|------|------|------|
| 数据开发治理 | **WeData** | 一站式 DataOps 平台 |
| 数据仓库 | CDWPG / TCHouse | 列存分析引擎 |
| 流处理 | Oceanus | 托管 Apache Flink |
| 数据湖 | DLC | Data Lake Compute, Serverless |
| 数据集成 | DataInLong | 基于 Apache InLong |
| BI 报表 | DataInsight | 可视化分析 |
| ML 平台 | TI Platform | 训练 + 推理 |
| 消息队列 | TDMQ / CKafka | Pulsar / Kafka 托管 |

> 基础设施：22 Region · 64 AZ · 3200+ CDN 节点 | 投入：CNY 5000亿 / 5年数字基建

#### WeData vs DataWorks vs Fabric Data Factory

| 维度 | 腾讯云 WeData | 阿里云 DataWorks | Microsoft Fabric DF |
|------|-------------|-----------------|-------------------|
| **定位** | 一站式数据开发治理 | 一站式大数据开发 | 统一数据集成/编排 |
| **开发模式** | 可视化拖拽 + SQL IDE | 可视化 + DataStudio | Pipeline + Dataflow Gen2 |
| **调度引擎** | DAG 编排 · 事件触发 · 跨项目依赖 | DAG · 事件 · 基线 | Pipeline Trigger + Activator |
| **支持引擎** | SparkSQL · DLC · TCHouse · EMR | MaxCompute · EMR · Hologres · Flink | Spark · SQL Warehouse · KQL |
| **任务类型** | 15+ (SparkSQL/Python/Shell 等) | 30+ | Pipeline Activities + Notebook |
| **数据治理** | 元数据 · 血缘 · 数据地图 · 质量规则 | 元数据 · 血缘 · 数据标准 · 质量 | Purview · OneLake Catalog · 血缘 |
| **多租户** | 项目级隔离 · 灵活权限 | 工作空间隔离 · RAM | Workspace · Capacity 隔离 |
| **AIOps** | 智能监控 · 告警 · 运维中心 | 基线 · 智能诊断 | Copilot · AI Functions |
| **国际化** | 22 Region (含海外) | 32 Region (国际更广) | 40+ Azure Region |
| **优势** | 游戏/社交垂直深耕 · 与腾讯生态打通 | 电商/金融场景成熟 · 国际客户多 | Power BI 用户基座 · SaaS 一体化 |
